...
namespace DXR
{
	DX11ConstantBuffer::DX11ConstantBuffer(uint32_t size, uint32_t binding) :m_BindingID(binding)
	{
		...
		DXR_DX_ASSERT(DX11Context::GetDevice()->CreateBuffer(&buffer, nullptr, m_ConstantBuffer.GetAddressOf()));
	...
	void DX11ConstantBuffer::SetData(const void* data, uint32_t size, uint32_t offset)
	{
		...
		DXR_DX_ASSERT(DX11Context::GetDeviceContext()->Map(m_ConstantBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource));
